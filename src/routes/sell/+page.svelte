<script lang="ts">
	import { getAuth, type AuthState } from '$lib/stores/auth.svelte';
	...
	async function deleteImage(image: GalleryImage) {
		...
		try {
			// Publish deletion event (kind 5) on Nostr
...
<div class="container mx-auto px-4 py-8 max-w-4xl">
	{#if !auth.isLoggedIn}
		<div class="text-center py-12">
			<h1 class="text-2xl font-bold text-white mb-2">Sell Your Photography</h1>
			<p class="text-gray-400">Sign in with Nostr to start selling.</p>
		</div>
	{:else}
		<h1 class="text-2xl font-bold text-white mb-2">Sell Your Work</h1>
